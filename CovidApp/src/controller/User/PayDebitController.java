	...
	public void actionPerformed(ActionEvent e) {
		...
		if (cm.equals("Thoát")) {
		    ...
		    try {
		    	...
		    	JOptionPane.showMessageDialog(null,"Cannot connect to server\nPlease check the server IP again!","Error",1);
		        view.dispose();
    ...
    public void run(){
        try {
            ...
            while((reader.readLine()) != null){
            	...
            	if(Managed_Payment.insertPayment(paymentHistory) && Managed_Payment.updateBalance(Double.parseDouble(pdview.getDebitCurrentText().getText()), 
    					Double.parseDouble(pdview.getBalanceCurrentText().getText()), Double.parseDouble(pdview.getPaymentMinimumText().getText()))) {

    						JOptionPane.showConfirmDialog(pdview, "Đã thanh toán thành công", "Thanh toán thành công", JOptionPane.CLOSED_OPTION, 1);
    						
    						Managed_User.showPaymentUser(pdview.getDebitCurrentText(), pdview.getBalanceCurrentText(), Managed_Account.setAccount(AccountCurrent.getUsernameCurrent()));
    						
    						pdview.getPaymentMinimumText().setText("");
    				}
    				else {
    					JOptionPane.showConfirmDialog(pdview, "Thanh toán không thành công", "Thanh toán không thành công", JOptionPane.CLOSED_OPTION, 0);
    				}
            }
        }
        catch (IOException ex) {
